...
///
fn send_msg<S: Deref<Target = UdpSocket>, A: ToSocketAddrs>(socket: &S,
                                                            addr: A,
                                                            payload: Payload,
                                                            ack_required: bool,
                                                            target: u64)
  ...
  let bytes = try!(socket.send_to(&encoded[..], addr).or(err!("failed to send message")));
...
impl<A> Bulb<A> where A: ToSocketAddrs
{
  ...
  ///
  pub fn send_msg_and_wait(&self, payload: Payload, ack_required: bool) -> Result<u8, String> {
...
impl Client {
  ...
  pub fn new<A: ToSocketAddrs>(addr: A) -> Result<Client, String> {
    ...
    let udp_builder = try!(UdpBuilder::new_v4().or(err!("failed to create builder")));
    let udp_socket = Arc::new(try!(udp_builder.bind(addr).or(err!("failed to bind to addr"))));
      ...
      while !closed.load(Ordering::SeqCst) {
        ...
        match *resp.payload() {
          Payload::Device(Device::StateService(Service::Udp, port)) => {
            ...
            devices.write().unwrap().entry(resp.target()).or_insert(Bulb {
              label: None,
              location: None,
              ip: src,
              port: port,
      ...
      while !closed.load(Ordering::SeqCst) {
        ...
        for d in devices.read().unwrap().values() {
          ...
          if !(options & GET_HOST_FIRMWARE).is_empty() {
            let _ = d.send_msg_and_wait(Payload::Device(Device::GetHostFirmware), false);
          }

          if !(options & GET_WIFI).is_empty() {
            let _ = d.send_msg_and_wait(Payload::Device(Device::GetWifiFirmware), false);
          }
        }
