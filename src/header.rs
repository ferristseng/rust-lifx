	...
	fn encode<S : Encoder>(&self, s: &mut S) -> Result<(), S::Error> {
		s.emit_struct(
			...
			|mut s| {
				...
				try!(s.emit_struct_field("origin_tagged_addressable_protocol", 1, 
					|mut s| { 
						let mut value = self.origin as u16;
						if self.tagged { value |= 0b0010_0000_0000_0000; }
						if self.addressable { value |= 0b0001_0000_0000_0000; }
				...
				try!(s.emit_struct_field(
					...
					|mut s| s.emit_seq(
						6, 
						|mut s| { 
							for i in 0..6 { try!(s.emit_seq_elt(i, |mut s| s.emit_u8(0))) } 
							Ok(()) 
						})));
				try!(s.emit_struct_field(
					"res1_ackreq_resreq", 
					...
					|mut s| {
						let mut value: u8 = 0;
						if self.ack_required { value |= 0b0000_0010; }
						if self.res_required { value |= 0b0000_0001; }
						s.emit_u8(value)
					}));
		...
		try!(d.read_struct(
			...
			|mut d| {
				...
				try!(d.read_struct_field(
					"size", 
					0,
					|mut d| d.read_u16().and_then(|v| { header.size = v; Ok(()) })));
				try!(d.read_struct_field(
				...
				try!(d.read_struct_field(
					"source",
					2,
					|mut d| d.read_u32().and_then(|v| { header.source = v; Ok(()) })));

				// FRAME ADDRESS
				try!(d.read_struct_field(
					"target",
					3,
					|mut d| d.read_u64().and_then(|v| { header.target = v; Ok(()) })));
				try!(d.read_struct_field(
					"res0",
					4,
					|mut d| d.read_seq(|mut d, _| {
						for i in 0..6 { try!(d.read_seq_elt(i, |mut d| d.read_u8())); } 
						Ok(())
					})));
				try!(d.read_struct_field(
					"res1_ackreq_resreq",
					5,
					|mut d| d.read_u8().and_then(|v| {
				...
				try!(d.read_struct_field(
					"sequence",
					6,
					|mut d| d.read_u8().and_then(|v| { header.sequence = v; Ok(()) })));
				...
				try!(d.read_struct_field(
					"type", 
					8, 
					|mut d| d.read_u16().and_then(|v| { header.typ = v; Ok(()) })));
...
fn test_header_encode_correctness() {
	...
	let correct = [
    0x24, 0x0, 0x0, 0x34, 0x29, 0xb9, 0x36, 0xa9, 0x0, 0x0, 0x0, 
